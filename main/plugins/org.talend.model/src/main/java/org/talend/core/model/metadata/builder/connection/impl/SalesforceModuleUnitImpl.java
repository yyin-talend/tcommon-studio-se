...
public class SalesforceModuleUnitImpl extends AbstractMetadataObjectImpl implements SalesforceModuleUnit {
			...
			if (metadataTable != oldMetadataTable) {
				...
				if (newMetadataTable.eInternalContainer() == null) {
					msgs = newMetadataTable.eInverseAdd(this,
							EOPPOSITE_FEATURE_BASE - ConnectionPackage.SALESFORCE_MODULE_UNIT__METADATA_TABLE, null,
							msgs);
				}
				if (msgs != null)
					msgs.dispatch();
	...
	public void setMetadataTable(MetadataTable newMetadataTable) {
		if (newMetadataTable != metadataTable) {
			...
			eNotify(new ENotificationImpl(this, Notification.SET,
					ConnectionPackage.SALESFORCE_MODULE_UNIT__METADATA_TABLE, newMetadataTable, newMetadataTable));
	}
	...
	public NotificationChain basicSetConnection(SalesforceSchemaConnection newConnection, NotificationChain msgs) {
		msgs = eBasicSetContainer((InternalEObject) newConnection, ConnectionPackage.SALESFORCE_MODULE_UNIT__CONNECTION,
				msgs);
		return msgs;
	}
	...
	public void setConnection(SalesforceSchemaConnection newConnection) {
		if (newConnection != eInternalContainer()
				|| (eContainerFeatureID() != ConnectionPackage.SALESFORCE_MODULE_UNIT__CONNECTION
						&& newConnection != null)) {
			...
			if (newConnection != null)
				msgs = ((InternalEObject) newConnection).eInverseAdd(this,
						ConnectionPackage.SALESFORCE_SCHEMA_CONNECTION__MODULES, SalesforceSchemaConnection.class,
						msgs);
