...
public class PropertiesDialog extends TitleAreaDialog {
	...
	private void updateProperties(List<Map<String, Object>> originalProperties,
			List<Map<String, Object>> referProperties) {
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (initPropertiesOfParent != null) {
			parentPropertiesTable = createPropertiesTable(propertiesBar, 0, getParentTitle(), initPropertiesOfParent,
					true, false);
		}
		int tableIndex = parentPropertiesTable == null ? 0 : 1;
		propertiesTableView = createPropertiesTable(propertiesBar, tableIndex, getTitle(), initProperties, isReadOnly(),
				true);
		updateExpandItems();
